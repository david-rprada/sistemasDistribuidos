...
function lista_expedientes(idMAP) {
    rest.get("/api/map/" + idMAP + "/expedientes", function (estado, respuesta) {
        ...
        for (var i = 0; i < respuesta.length; i++) {
            ...
            var celdaBoton2 = fila.insertCell(datos.length);
            ...
            celdaBoton2.innerHTML = "<button onclick='chat(" + respuesta[i].id + ")'>Chat</button>";
        }
    });
}

//NO SE SI ESTA BIEN ESTO HYA QUE PREGUNTAR.
function chat(idExpediente){
    cambiarSeccion("chat_map");

    var tablaContainer = document.getElementById("lista_mensajes_map");
    ...
    // Crear la tabla
    var tabla = document.createElement("table");
    tabla.setAttribute("id", "tabla_mensajes");

    // Encabezados de la tabla
    var encabezados = ["Fecha / Hora", "Nombre del médico", "Mensaje", ""];
    ...
    datos.forEach(function(dato) {
        ...
        var fechaHoraCell = fila.insertCell();
        fechaHoraCell.textContent = dato.fechaHora;
        var nombreMedicoCell = fila.insertCell();
        nombreMedicoCell.textContent = dato.nombreMedico;
        var mensajeCell = fila.insertCell();
        mensajeCell.textContent = dato.mensaje;
    });
